 ...
 *
 * 自动补齐的**块间顺序**（v3.7.8）：
 *   聊天记录永远补在**最后**，其余按 SYSTEM_BLOCKS 原顺序补在它前面。
 *   理由见下面 tailOrder 那段注释 —— 一句话：聊天记录每轮都在变长，
 *   排在它后面的东西就每轮都掉出前缀缓存。
 *
 ...
 *   那些条目就两边都没有了。
 *   ⚠️ 它只决定**块的先后**，不决定「哪几块进得来」—— 那是白名单的事。
 *
 * @param {{
...
export function buildSystemPrompt(p) {
     ...
     * 补齐顺序：**聊天记录永远放最后**。
     * ============================================================
     * 前缀缓存只认「最长相同前缀」，一旦某个 token 分叉，它后面的全部按
     * 未命中价重算；而 DeepSeek 的缓存命中价只有未命中的 1/50 ~ 1/120
     * （v4f 0.02 vs 1 元/M，v4p 0.025 vs 3 元/M）。
     *
     * 素材里只有聊天记录是「每轮都在变长」的块，所以它必须排在最后 ——
     * 排在它后面的东西，每轮都会掉出缓存前缀、按未命中价重算一遍。
     * 以前那份默认模板把 {{chat_history}} 写在最后一行，正好是对的；
     * 模板空着走自动补齐时，SYSTEM_BLOCKS 里 history 排第 0，就反了。
     *
     * ⚠️ 只在这里排序，**不要改 SYSTEM_BLOCKS 数组本身** ——
     * 那个数组同时决定「白名单」页表格「系统预设条目」组的行序
     * （panel.js:1271）以及素材状态摘要的拼接顺序（panel.js:889）。
     * 在这里排一次序，配置和界面都不受影响。
     *
     * 用户写在模板里的占位符位置永远优先（上面的循环已经跳过它们），
     * 这里只决定「模板里没写的那几块」的先后。
     */
    const tailOrder = SYSTEM_BLOCKS
        .filter(b => b.key !== 'history')
        .concat(SYSTEM_BLOCKS.filter(b => b.key === 'history'));

    for (const b of tailOrder) {
