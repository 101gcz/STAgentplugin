...
function paneTemplates() {
    ...
    <div class="dro-pane" id="dro-pane-status">
        ...
        <details class="dro-adv" id="dro-request-map-box">
            ...
            <div class="dro-hint" id="dro-request-map">还没有数据。发一条消息，这里会列出主模型这一轮收到的每一条消息。</div>
        </details>
    </div>
    ...
    <div class="dro-pane" id="dro-pane-api">
        ...
        <div class="dro-sec">主模型历史裁剪</div>
        <label class="checkbox_label" for="dro-trim-history">
            <input type="checkbox" id="dro-trim-history">
            <span>主模型只留最近几轮对话</span>
        </label>
        <div class="dro-inline">
            <label for="dro-trim-keep">保留最近</label>
            <input type="number" id="dro-trim-keep" class="text_pole" min="1" max="100" style="width:70px">
            <span>轮</span>
        </div>
        <div class="dro-hint" id="dro-trim-hint" title="裁剪发生在酒馆扫完世界书、拼好 messages 之后、发请求之前。"></div>
        <div class="dro-hint">
            一轮 = 一条 AI 输出 + 它之后你说的那些话（默认 1 轮）。<br>
            裁完之后主模型读到的就是 <strong>最近一轮对话 + 大纲</strong>，
            旧剧情全部由大纲承担 —— 这是唯一能真正砍掉主模型 token 的地方。<br>
            「聊天记录」素材块设成<strong>只给大纲</strong>时这里<strong>自动生效</strong>（本来就是同一件事）；
            设成「都给 / 只给主模型」时不动手，除非勾上这个开关。<br>
            裁剪只认「整条正文与那一层完全一致」的消息，认领不到的一律保留；
            <strong>最后一条 user 消息永不删</strong>；删掉的条数绝不会超过该裁的层数。
        </div>
    ...
    <div class="dro-pane" id="dro-pane-whitelist">
        ...
        <details class="dro-adv">
            <summary>「去向」怎么理解（三档都会真的执行）</summary>
            <div class="dro-hint">
                <strong>只给大纲</strong>：进大纲提示词，<strong>并且从主模型请求里删掉</strong>。<br>
                　· 预设条目 / 世界书 / 角色卡 / Persona / 对话示例：整条精确匹配才删，
                匹配不上就保留并记日志（酒馆可能把它们和别的东西合并成了一条）。<br>
                　· <strong>聊天记录</strong>：主模型那边只留<strong>最近几轮</strong>
                （轮数在「调用」页设，默认 1 轮）—— 全删会让你刚说的话都进不去。<br>
                <strong>只给主模型</strong>：不进大纲，主模型那边保持原样。<br>
                <strong>都给</strong>：两边都有。<br>
                <br>
                两组用的是同一套行、同一套规则，全选 / 全清 / 统一设为对两组一视同仁。
                酒馆里关掉的条目会置灰、不可勾选。
            </div>
        </details>
...
function cacheElements() {
    const ids = [
        ...
        'dro-trim-history', 'dro-trim-keep', 'dro-trim-hint',
...
function bindSettings() {
    ...
    // ---- 阶段二：主模型历史裁剪 ----
    on(els['dro-trim-history'], () => {
        setSettings({ trimHistory: els['dro-trim-history'].checked === true });
        renderTrimHint();
    });
    on(els['dro-trim-keep'], () => {
        setSettings({ trimKeepRounds: Number(els['dro-trim-keep'].value) });
        renderTrimHint();
    });
...
function blockFacts() {
    ...
    return {
        history: {
            ...
            brief: `聊天记录 ${hs.floors} 层 / ${fmtNum(hs.finalChars)} 字`,
            off: '聊天记录 不给大纲',
            toMain: '聊天记录 只给主模型',
        },
        worldInfo: {
...
function whitelistRow(spec) {
    ...
    sel.title = spec.selTitle || '只给大纲 = 拼进大纲提示词；只给主模型 = 不进大纲；都给 = 两边都要';
...
function blockRow(b, fact, cfg, presetName) {
    ...
    return whitelistRow({
        ...
        checkTitle: '不勾 = 这块不给大纲模型（去向里的「只给主模型」也是这个意思）',
        selTitle: '三档都真的会执行：\n' +
            '只给大纲 = 进大纲提示词，并且从主模型请求里删掉（聊天记录则裁到只留最近几轮）；\n' +
            '只给主模型 = 不进大纲；\n' +
            '都给 = 两边都有。',
...
export function refreshForms() {
    ...
    setChecked('dro-trim-history', s.trimHistory === true);
    setValue('dro-trim-keep', s.trimKeepRounds);
...
/**
 * 「主模型历史裁剪」那一行实时说明：现在会裁掉几轮、省多少。
 * 数字取自上一次运行的真实结果（lastTrim.history），拿不到就按配置预估。
 * 注意：素材块「聊天记录」= 只给大纲 时裁剪会自动生效，
 * 所以这里的「生效中」不一定等于那个开关被勾上。
 */
function renderTrimHint() {
    const box = els['dro-trim-hint'];
    if (!box) return;
    const s = getSettings();
    const rounds = Math.max(1, Math.min(100, Math.round(Number(s.trimKeepRounds)) || 1));
    const active = wantTrimHistory(s);
    if (!active) {
        box.textContent = '当前：不裁 —— 主模型收到多少历史由酒馆的上下文预算决定。';
        return;
    }
    const t = lastTrimStats();
    const h = t && t.history;
    if (h && h.removed > 0) {
        box.textContent = `上一次真的裁了：删掉 ${h.removed} 条旧楼层 / ${fmtNum(h.removedChars)} 字，` +
            `主模型只留最近 ${h.rounds} 轮（原 ${h.totalFloors} 层）。`;
    } else if (h && !h.skipped && h.removed === 0) {
        box.textContent = '上一次生效了但一条都没认领到（历史没被裁）—— 见「日志」页的警告。';
    } else if (h && h.skipped) {
        box.textContent = `当前：生效中（保留最近 ${rounds} 轮）｜上次未裁：${h.skipped}`;
    } else {
        box.textContent = `当前：生效中 —— 主模型只留最近 ${rounds} 轮，更早的楼层会删掉。`;
    }
}
...
function renderStats() {
    ...
    const rows = [
        ...
        kv('聊天正文（原文）', fmtNum(chat.rawChars) + ' 字   ≈ ' + fmtNum(chat.tokens) + ' tokens'),
        kv('送大纲的聊天记录', fmtNum(hs.finalChars) + ' 字'),
        kv('世界书（本轮）', (world.count || world.text)
            ? `激活 ${world.count} 条 / ${fmtNum(world.chars)} 字` +
              (world.overflowed ? '（预算溢出）' : '') +
              (world.stale ? '（已过期，没拼进提示词）' : '')
            : '本轮没有激活条目'),
        kv('主模型历史裁剪', trimHistoryRow()),
        kv('主模型请求整形', trimRow()),
    ];
...
function trimHistoryRow() {
    const s = getSettings();
    if (!wantTrimHistory(s)) return '不裁（主模型收到多少历史由酒馆的上下文预算决定）';
    const why = s.trimHistory === true ? '开关打开' : '「聊天记录」= 只给大纲';
    const t = lastTrimStats();
    const h = t && t.history;
    if (!h) return `生效中（${why}）｜保留最近 ${s.trimKeepRounds} 轮｜还没跑过`;
    if (h.skipped) return `生效中｜上次未裁：${h.skipped}`;
    if (h.removed > 0) {
        return `生效中｜上次删掉 ${h.removed} 条 / ${fmtNum(h.removedChars)} 字｜` +
            `只留最近 ${h.rounds} 轮（原 ${h.totalFloors} 层）`;
    }
    return '生效中｜上次一条都没认领到（历史没被裁，见日志）';
}
...
function trimRow() {
    ...
    parts.push(t.removed ? `已移除 ${t.removed} 条 / ${fmtNum(t.removedChars)} 字` : '条目没有可移除的');
    if (t.missed.length) parts.push(`条目未命中 ${t.missed.length} 条（已保留）`);
    if (t.blocks) {
        parts.push(t.blocks.removed
            ? `素材块已移除 ${t.blocks.removed} 条 / ${fmtNum(t.blocks.removedChars)} 字（${t.blocks.names.join('、')}）`
            : '素材块没有可移除的');
        if (t.blocks.missed.length) parts.push(`素材块未命中 ${t.blocks.missed.join('、')}`);
    }
    return parts.join('｜');
}
...
export function reportPreview(info) {
    ...
    log(
        ...
        `｜白名单命中 ${info.presetPicked} 条｜聊天记录 ≈ ${fmtNum(info.historyTokens)} tokens`,
        'info', 'panel'
    );
...
export function reportRequestMap(d) {
    ...
    log(`主模型本轮真实组成｜${describeRequestText(d)}`, 'info', 'panel');

    // 有「只给大纲还留着」的就必须是警告级，别让它淹在 info 里
    if (d.warnings.length && d.warnings[0].startsWith('★')) log(d.warnings[0], 'warn', 'panel');
}

function describeRequestText(d) {
    return `共 ${d.total.count} 条 / ${fmtNum(d.total.chars)} 字｜` +
        d.byKind.slice().sort((a, b) => b.chars - a.chars)
            .map(k => `${k.label} ${k.count}条/${fmtNum(k.chars)}字`).join('｜');
}
