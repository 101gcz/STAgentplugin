    ...
    root.innerHTML = `
        ...
        <div id="dro-panel" class="dro-panel">
            <div class="dro-panel-head" id="dro-panel-head">
                <span class="dro-panel-title">五号预设Agent</span>
...
function blockFacts() {
    ...
    return {
        ...
        worldInfo: {
            ...
            brief: world.text
                ? `世界书 本轮激活 ${world.count} 条 / ${fmtNum(world.chars)} 字（${Math.round(world.ageMs / 1000)}s 前的扫描）` +
                  (world.overflowed ? '｜⚠️ 预算溢出' : '')
                : (world.stale ? '世界书 扫描结果已过期（这一轮可能没有激活条目）' : '世界书 本轮没有激活任何条目'),
            off: '世界书 不给大纲',
            toMain: '世界书 只给主模型',
        },
        charCard: {
...
function renderStats() {
    ...
    const rows = [
        ...
        kv('酒馆正则', chat.regexAvailable ? '可用（大纲用的历史会过它压缩）' : '不可用（跳过压缩）'),
        kv('世界书（本轮）', world.text
            ? `激活 ${world.count} 条 / ${fmtNum(world.chars)} 字` + (world.overflowed ? '（预算溢出）' : '')
            : (world.stale ? '扫描结果已过期' : '本轮没有激活条目')),
        kv('主模型请求整形', trimRow()),
    ];
