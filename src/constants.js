...
export const DEFAULTS = {
    ...
    /** ---- 裁剪发给主模型的历史 ---- */
    /**
     * 独立开关：主模型只留最近 N 轮对话，更早的楼层从主模型请求里删掉。
     *
     * 它和「聊天记录」素材块的**去向**是两件事，但会叠加：
     *   · 素材块「聊天记录」= 只给大纲（默认）时，裁历史**自动生效** ——
     *     「只给大纲」的承诺就是「主模型不再读它」，而对聊天记录来说
     *     唯一合理的删法就是「只留最近几轮」：全删会让主模型看不到你刚说的话。
     *   · 素材块 = 都给 / 只给主模型时不动手，除非这个开关打开。
     */
    trimHistory: false,
    /**
     * 主模型保留最近多少**轮**对话。
     * 一轮 = 一条 AI 输出 + 它之后你说的那些话（通常就是 1 条 AI + 1 条 user）。
     * 默认 1 轮：旧剧情全交给大纲，主模型只负责接着最近这一轮往下写。
     */
    trimKeepRounds: 1,
    /**
     * 【历史字段，只为兼容旧配置】v3.9.0 之前按「层」算（默认 10 层）。
     * 现在按「轮」算，这个字段不再参与裁剪，留着不删以免老配置出问题。
     */
    trimKeepFloors: 10,
};
