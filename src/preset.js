 ...
 *
 *  3. 原版调用的 selectActivatedEntries 并不属于酒馆助手，
 *     它是 ST-Prompt-Template 扩展的内部函数。在只装酒馆助手的
 *     环境里会抛 ReferenceError，然后被 catch 吞掉，世界书功能
 *     永久失效且毫无提示。所以本版所有「能力缺失」都必须报出来。
 *
 * ★ v3.7.8 删掉了「引用完整性检查」（条目之间 <Tag> 互相引用的检查）。
 *   面板上那块早在 v3.7.7 就按用户要求删了，这里这份是同一套逻辑的
 *   另一份实现，只往日志里写「「X」引用了 <Y>…」。现在两处都没了。
 *   buildEntryBlock 仍然会返回 warnings，但只剩「宏展开失败」这一种。
 */
